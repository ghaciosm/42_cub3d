...
/*   Updated: 2023/09/20 14:20:49 by myagiz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start(t_data *data)
{
	...
	data->img_rays_addr = mlx_get_data_addr(data->img_rays,
			&bits_per_pix, &size_lin, &endia);
	data->maps->lengths = (int *)malloc(sizeof(int) * 4);
}
...
void	start_window(t_data *data)
{
	data->mlx = mlx_init();
	if (!data->mlx)
		error(1, "Error initializing MiniLibX", data, 5);
	data->win = mlx_new_window(data->mlx, WINDOW_WIDTH, WINDOW_HEIGHT, "cub3d");
	if (!data->win)
		error(1, "Error creating window", data, 5);
	find_player(data);
}
