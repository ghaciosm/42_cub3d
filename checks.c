...
/*   Updated: 2023/09/19 11:38:58 by ghaciosm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++i < data->height)
	{
		...
		while (++j < data->width)
		{
				...
				|| data->map[i][j] == '\0')
			{
				...
				{
					if (flag)
						error(1, "There is more than one player!", data);
					flag = 1;
				}
			}
			else
				error(1, "Wrong character on map!", data);
		}
	}
	if (!flag)
		error(1, "No character on map!", data);
}
...
void	map_check(t_data *data)
{
	...
	while (data->map[height])
	{
		if (ft_strlen(data->map[height]) < data->width)
			add_space(&data->map[height], data->width);
		height++;
	}
